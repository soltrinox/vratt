...
@interface TreasureHuntViewController ()<TreasureHuntRendererDelegate, OEEventsObserverDelegate> {
...
//  NSString *SKSServerUrl = @"nmsps://NMDPTRIAL_soltrinox20130220084958@sslsandbox-nmdp.nuancemobility.net:443";
//  NSString *SKSAppKey = @"NMDPTRIAL_soltrinox20130220084958";
//
//  SKSession* session = [[SKSession alloc] initWithURL:[NSURL URLWithString:SKSServerUrl] appToken:SKSAppKey];
//
//  NSDictionary *options = @{
//          SKTransactionResultDeliveryKey: SKTransactionResultDeliveryFinal,
//          SKTransactionManageCategoryKey: @"NO",
//  };
//
...
- (void) pocketsphinxDidReceiveHypothesis:(NSString *)hypothesis recognitionScore:(NSString *)recognitionScore utteranceID:(NSString *)utteranceID {
    ...
    if([hypothesis isEqualToString:@"fire"]) {
        if( _treasureHuntRenderer.is_cube_focused ){
            [_treasureHuntRenderer spawnCube];
        }
...
- (void)startSpeeking {
    ...
    // Here is an example of dynamically creating an in-app vocabulary to switch to.
    ...
    // words will be slower to generate. You can use this information to give your users some UI feedback about what the expectations for wait times should be.
    
    
    // generateLanguageModelFromArray:withFilesNamed returns an NSError if there was an error.
    ...
    if(error) {
        ...
        NSLog(@"\n\nDict words:\n%@,\nand if you say \"change model\" second Dict words:\n%@", firstLanguageArray, secondLanguageArray);
